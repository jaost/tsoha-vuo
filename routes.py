...
def create_feed():
    ...
    if request.method == "POST":
        ...
        if len(error) == 0 and feeds.create_feed(secret, title, description):
            ...
            return render_template("new_feed.html", secret=secret, title=title, description=description, error=error)
...
def create_item(secret):
    return render_template("new_item.html", secret=secret) ##, image=request.args.get('image'), description=request.args.get('description'), error=request.args.get('error'))
    ...
    if request.method == "POST":
        if request.form.get("action", False) == "delete":
            if feeds.delete_feed(secret):
                return redirect("/")
            return render_template("feed.html", secret=secret, error=["Could not delete item."])
            ...
            if len(description) < 3 or len(description) > 199:
                error.append("Description should be more than 3 and under 200 characters.")
            ...
            if len(error) > 0:
                return render_template("new_item.html", secret=secret, image=request.files['image'], description=description, error=error)
            ...
            if feeds.create_item(secret, file_path, description):
                ...
                return render_template("new_item.html", secret=secret, error=["Something went wrong. Try again later."])
        else:
...
def login():
    ...
    if request.method == "POST":
        ...
        if users.login(username,password):
            ...
            return render_template("login.html", username=username, error=["Wrong username / password"])
